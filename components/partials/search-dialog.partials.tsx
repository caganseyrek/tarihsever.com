...
const CommandRoot = ({ ref, className, children, ...props }: React.ComponentProps<typeof Command>) => {
  return (
    <Command
      ref={ref}
...
const CommandList = ({ ref, className, children, ...props }: React.ComponentProps<typeof Command.List>) => {
  return (
    <Command.List ref={ref} className={cn("max-h-[300px] overflow-y-auto overflow-x-hidden", className)} {...props}>
      {children}
    </Command.List>
  );
};

const CommandEmpty = ({ ref, className, children, ...props }: React.ComponentProps<typeof Command.Empty>) => {
  return (
    <Command.Empty ref={ref} className={cn("py-6 text-center", className)} {...props}>
      {children}
    </Command.Empty>
  );
};

const CommandSeparator = ({ ref, className, children, ...props }: React.ComponentProps<typeof Command.Separator>) => {
  return (
    <Command.Separator ref={ref} className={cn("-mx-1 h-px bg-container-border", className)} {...props}>
      {children}
    </Command.Separator>
  );
};

const CommandInput = ({ ref, placeholder }: React.ComponentProps<typeof Command.Input>) => {
  return (
